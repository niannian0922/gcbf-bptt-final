...
Supports dual-policy (pilot/guardian) architectures.
"""
...
# --- THE CORE FIX: Import the CORRECT, full-featured policy class ---
...
class ModelEvaluator:
    ...
    def __init__(self, config: Dict[str, Any], device: torch.device, pilot_path: str, guardian_path: str):
        """
        Initialize the model evaluator for dual-policy evaluation.
        """
        self.config = config
        self.device = device
        ...
        # --- UNIFIED AND CORRECT POLICY INSTANTIATION ---
        # Both policies are initialized with the full configuration object,
        # ensuring they have access to all necessary parameters ('policy', 'losses', etc.)
        ...
        self.guardian_policy = BPTTPolicy(config) # Guardian shares the same architecture
    ...
    def _evaluate_single_episode(self, seed: int = None) -> EpisodeResult:
        ...
        while episode_length < self.env.max_steps:
            ...
            with torch.no_grad():
                # We evaluate the performance of the PILOT policy
            ...
            if step_result.cost.item() > 0: # Collision
                status = 'collision'
                break
            
            if self.env.get_goal_distance(next_state).item() < self.env.agent_radius: # Success
                status = 'success'
                episode_length += 1
                break
    ...
    def print_final_report(self, kpis: Dict[str, Any]):
        ...
        print(f"\n🔧 MODEL CONFIGURATION:")
        print(f"   Pilot Model: {self.pilot_path}")
        print(f"   Guardian Model: {self.guardian_path}")
